...
static void read_config(const char * path) {
	...
	/* Backward compatibility mode: Use fixed memory sizes
	 * Compatible with legacy OS simulation config files that don't have
	 * memory size configuration line.
	 * Input format: [time slice] [N = Number of CPU] [M = Number of Processes to be run]
	 */
        memramsz  =  0x10000000;  /* 256MB RAM */
        memswpsz[0] = 0x1000000;  /* 16MB Swap 0 */
	for(sit = 1; sit < PAGING_MAX_MMSWP; sit++)
		memswpsz[sit] = 0;  /* Other swaps disabled */
#else
	/* Custom memory size mode: Read memory sizes from input file
	 * Input format: [RAM_SZ] [SWP_SZ0] [SWP_SZ1] [SWP_SZ2] [SWP_SZ3]
	 * Requirements:
	 *   - RAM_SZ must be > 0 (positive integer)
	 *   - At least one SWP_SZ must be > 0 (at least one swap must be active)
	 *   - Remaining SWP_SZ can be 0 (swap deactivated)
	 *   - All values are non-negative integers
	 */
	fscanf(file, "%d", &memramsz);
	for(sit = 0; sit < PAGING_MAX_MMSWP; sit++)
		fscanf(file, "%d", &(memswpsz[sit])); 
	fscanf(file, "\n"); /* Consume newline after memory sizes */
	
	/* Validate memory configuration */
	if (memramsz <= 0) {
		printf("Error: MEMRAM size must be positive (got %d)\n", memramsz);
		exit(1);
	}
	int active_swp_count = 0;
	for(sit = 0; sit < PAGING_MAX_MMSWP; sit++) {
		if (memswpsz[sit] < 0) {
			printf("Error: MEMSWP[%d] size must be non-negative (got %d)\n", sit, memswpsz[sit]);
			exit(1);
		}
		if (memswpsz[sit] > 0) {
			active_swp_count++;
	...
	if (active_swp_count == 0) {
		printf("Error: At least one MEMSWP must be active (size > 0)\n");
		exit(1);
	}
#endif
#endif
