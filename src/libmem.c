...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  if (!PAGING_PAGE_PRESENT(pte)) {
      printf("Error: pg_getpage - PGN %d invalid\n", pgn);
      return -1;
  }

  if (PAGING_PTE_SWAPPED_MASK & pte) {
      printf("Page Fault: PGN %d đang ở SWAP. Bắt đầu Swap In...\n", pgn);
      addr_t vicpgn, swpfpn_victim, tgtfpn;
      
      // SỬA: Dùng caller->mm
      if (find_victim_page(caller->mm, &vicpgn) < 0) {
          // Nếu không tìm thấy nạn nhân, thử lấy RAM trống
          ...
          printf("Error: RAM đầy và không tìm thấy nạn nhân!\n");
          return -1;
      }
      ...
      if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn_victim) < 0) {
          printf("Error: SWAP đầy!\n");
          return -1;
      }

      printf("SWAP OUT: PGN %u (FPN %u) -> SWAP_FPN %u\n", vicpgn, vicfpn, swpfpn_victim);
      ...
      printf("SWAP IN: SWAP_FPN %u -> PGN %d (FPN %u)\n", swpfpn_target, pgn, tgtfpn);
